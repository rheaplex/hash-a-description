...
const HASH: [&str; 6] = ["hash", "cryptographic hash", "sha256", "SHA256", "sha256 hash", "SHA256 hash"];
const HASH_ACTION: [&str; 5] = ["hashing", "cryptographic hashing", "cryptographically hashing", "sha256 hashing", "SHA256 hashing"];
...
const HASH_RESULT: [&str; 5] = ["a string", "a digest", "a hexadecimal string", "a message digest", "a hexadecimal message digest"];
        ...
        for subject in SUBJECT {
            for result in RESULT {
                for hash_result in HASH_RESULT {
                    for relation in RELATION {
                        let prefix = format!("{} {} {} {} that {}", hash_action, subject, result, hash_result, relation);
...
fn maybe_find_more_than_one (prefixes: &Vec<String>) {
    for digit in 0..15 {
        let matcher = Regex::new(format!(r"{:x}{{2,}}", digit).as_str()).unwrap();
        for qualifier in ["more than one", "multiple"] {
            maybe_find_feature(&format!("{} {:x}", qualifier, digit), &matcher, prefixes);
...
fn maybe_find_n (prefixes: &Vec<String>) {
    for digit in 0..15 {
        for count in 5..15 {
            let matcher = Regex::new(format!(r"{:x}", digit).as_str()).unwrap();
            maybe_find_feature_count(&format!("{} {:x}s", count, digit), &matcher, count, prefixes);
            // Use uppercase hex digits as well.
            if digit > 9 {
                maybe_find_feature_count(&format!("{} {:X}s", count, digit), &matcher, count, prefixes);
            }
...
fn maybe_find_n_successive (prefixes: &Vec<String>) {
    for n in 0..15 {
        for m in 5..15 {
            let matcher = Regex::new(format!(r"{:x}{{{},}}", n, m).as_str()).unwrap();
            maybe_find_feature(&format!("{} successive {:x}s", m, n), &matcher, prefixes);
            maybe_find_feature(&format!("{} successive {:X}s", m, n), &matcher, prefixes);
            maybe_find_feature(&format!("{} {:x}s successively", m, n), &matcher, prefixes);
            maybe_find_feature(&format!("{} {:X}s successively", m, n), &matcher, prefixes);
        }
    }
}
...
fn maybe_find_n_and_m_successive (prefixes: &Vec<String>) {
    for digit1 in 0..15 {     
        for digit2 in 0..15 {
            if digit1 != digit2 {
                for count1 in 3..15 {  
                    for count2 in 3..15 {
                        let matcher = Regex::new(format!(r"{:x}{{{}}}.*{:x}{{{}}}", digit1, count1, digit2, count2).as_str()).unwrap();
                        maybe_find_feature(&format!("{} {:x}s and {} {:x}s", count1, digit1, count2, digit2), &matcher, prefixes);
                    }
                }
            }
        }
